...
global x, y, m, r, R, c1, c2, r1, R1, c1h, c2h, rn
p = 65537
g = 3
...
def submitRandh():
    ...
    r1 = InputRandh.get()
    rn = int(r) + int(r1)
    R1 = gmpy2.powmod(int(y), int(rn), int(p))
    print("User entered random number:", r1)


def displayC1h():
    global c1h
    c1h = gmpy2.powmod(int(g), int(rn), int(p))
    subheadDisplayC1h.configure(text=f"{c1h}")


def displayC2h():
    global c2h
    c2h = gmpy2.f_mod(int(m) * int(R1), int(p))
    subheadDisplayC2h.configure(text=f"{c2h}")


def decrypt1():
    div = gmpy2.invert(int(R1), int(p))
    print("div =", div)
    numerator = c2h * div
...
head.set("ElGamal Re-Encryption")
label.pack()
frame = Frame(top)
disp = StringVar()
...
frameRE = Frame(top)
dispREnc = StringVar()
subheadRE = Label(frameRE, textvariable=dispREnc)
dispREnc.set("5. Re-Encrypt")
subheadRE.pack(side="left")
frameRE.pack(anchor="w")

# Choose Random number r'
frameRandh = Frame(top)
ranh = StringVar()
subheadRandh = Label(frameRandh, textvariable=ranh)
ranh.set("5.1 Choose a random number r'     r' =")
subheadInputRandh = Label(frameRandh, text="", relief="solid", width=250)

# Create an entry widget
InputRandh = Entry(subheadInputRandh)

# Create a button to submit the input
BInputRandh = Button(frameRandh, text="Submit", command=submitRandh)

subheadRandh.pack(side="left")
subheadInputRandh.pack(side="left")
InputRandh.pack(side="left")
BInputRandh.pack(side="left")
frameRandh.pack(anchor="w", padx=10)

# Compute c1'
frameComputeC1h = Frame(top)
C1h = StringVar()
subheadC1h = Label(frameComputeC1h, textvariable=C1h)
C1h.set("5.2 Compute c1'=g^r+r' mod p")

BComputeC1h = Button(frameComputeC1h, text="Com", command=displayC1h)
dispC1h = Label(frameComputeC1h, text="c1'=")
subheadDisplayC1h = Label(frameComputeC1h, text="", relief="solid", width=250)

subheadC1h.pack(side="left")
BComputeC1h.pack(side="left")
dispC1h.pack(side="left")
subheadDisplayC1h.pack(side="left")
frameComputeC1h.pack(anchor="w", padx=10)

# Compute c2
frameComputeC2h = Frame(top)
C2h = StringVar()
subheadC2h = Label(frameComputeC2h, textvariable=C2h)
C2h.set("5.3 Compute c2'=m* y^r+r' mod p")

BComputeC2h = Button(frameComputeC2h, text="Com", command=displayC2h)
dispC2h = Label(frameComputeC2h, text="c2'=")
subheadDisplayC2h = Label(frameComputeC2h, text="", relief="solid", width=250)

subheadC2h.pack(side="left")
BComputeC2h.pack(side="left")
dispC2h.pack(side="left")
subheadDisplayC2h.pack(side="left")
frameComputeC2h.pack(anchor="w", padx=10)

# Decryption
frameD = Frame(top)
dispDec = StringVar()
subheadD = Label(frameD, textvariable=dispDec)
dispDec.set("5.1 Decrypt C'=(c1',c2' )")
...
M.set("m =c2'/(y ^ r + r')  mod p")

BComputeM = Button(frameComputeM, text="Com", command=decrypt1)
